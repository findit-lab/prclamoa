...
import { getArchiveCopy, getArchiveUI } from "@/data/archive";
...
export function InfluencerArchive({ locale = "ko" }: { locale?: Locale }) {
  const ui = getArchiveUI(locale);
  const total = influencerImages.length;
  const serviceHref =
    locale === "ko" ? "/services/influencer-pr" : `/${locale}/services/influencer-pr`;

  return (
    <main className="min-h-screen bg-surface text-deep-ink">
      <ArchiveTabs variant="light" locale={locale} rightSlot={`${total} POSTS`} />

      {/* Profile header — Instagram style */}
      <section className="px-5 md:px-12 pt-16 md:pt-24 pb-10 md:pb-14 border-b border-deep-ink/10">
        <span className="text-label-caps text-secondary block mb-6">
          — ARCHIVE / 04 · @CLAMOA.INFLUENCER
        </span>
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-8 md:gap-14">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="space-y-1 text-sm md:text-base leading-relaxed max-w-2xl">
              ...
              <p className="text-secondary">{ui.influencerBio}</p>
              <p className="text-secondary">{ui.influencerLocation}</p>
              <p>
                <a href={serviceHref} className="text-[#1e3a8a] hover:underline font-medium">
                  clamoa.com{serviceHref}
                </a>
              </p>
            </div>
          </div>
      ...
      <section className="px-5 md:px-12 py-16 md:py-24 border-t border-deep-ink/10 bg-surface-low/40">
        ...
        <h2 className="sr-only">{ui.featuredHeading}</h2>
        <div className="max-w-xl mx-auto bg-surface border border-deep-ink/10 rounded-sm overflow-hidden shadow-sm">
          ...
          <div className="px-4 pt-3 pb-4">
            ...
            <p className="text-sm leading-relaxed">
              <span className="font-medium">clamoa.influencer</span> {ui.featuredCaption}
      ...
      <ArchiveCTA locale={locale} slug="influencer" />
    </main>
  );
}

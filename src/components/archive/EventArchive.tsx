...
// deterministic pseudo-random rotation / offset
...
export function EventArchive({ locale = "ko" }: { locale?: Locale }) {
  const copy = getArchiveCopy(locale, "event");
  ...
  return (
    <main className="min-h-screen bg-surface text-deep-ink">
      <ArchiveTabs variant="light" locale={locale} rightSlot={`${imgs.length} FRAMES`} />

      {/* HERO — field note header */}
      <section className="px-5 md:px-12 pt-16 md:pt-24 pb-10 md:pb-16 border-b-2 border-deep-ink relative overflow-hidden">
        <span
          aria-hidden
      ...
      <section className="relative px-5 md:px-12 py-16 md:py-24 border-b-2 border-deep-ink bg-[radial-gradient(circle_at_20%_10%,rgba(0,0,0,0.04),transparent_50%),radial-gradient(circle_at_80%_90%,rgba(0,0,0,0.04),transparent_50%)]">
        <div className="flex items-end justify-between mb-10 md:mb-14">
          ...
          <span className="hidden md:block text-[10px] font-mono tracking-[0.15em] text-secondary max-w-[200px] text-right">
            {ui.eventScrapNote}
          </span>
        </div>
      ...
      <section className="bg-deep-ink py-12 md:py-20 px-5 md:px-12">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-1.5 md:gap-2">
          {contact.map((img, i) => {
            ...
            return (
              <figure key={img.id} className="relative aspect-square overflow-hidden bg-black group">
                <span
                  aria-hidden
      ...
      <ArchiveCTA locale={locale} slug="event" />
    </main>
  );
}
